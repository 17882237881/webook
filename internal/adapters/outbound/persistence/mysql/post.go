...
func (d *PostDAO) CountByAuthor(ctx context.Context, authorId int64) (int64, error) {
	var count int64
	err := d.db.WithContext(ctx).Model(&Post{}).Where("author_id = ?", authorId).Count(&count).Error
	return count, err
}
...
// Count 统计已发布帖子总数
func (d *PublishedPostDAO) Count(ctx context.Context) (int64, error) {
	var count int64
	err := d.db.WithContext(ctx).Model(&PublishedPost{}).Count(&count).Error
	return count, err
}
